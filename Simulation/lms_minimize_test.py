...
# 生成指定角度范围的圆柱面上的点云
def generate_partial_cylinder_points(center, axis, radius=1.0, height=2.0, angle_range=60, resolution=500,resolution_theta=500,resolution_z=500, noise_stddev=0.01):
    # 归一化轴向量
    axis = axis / np.linalg.norm(axis)

    
    # 创建指定角度范围的圆柱面上的点云
    theta = np.linspace(0, np.deg2rad(angle_range), resolution_theta)  # 角度范围从0到指定角度（以度为单位）
    z = np.linspace(-height / 2, height / 2, resolution_z)  # 高度范围从-高度/2到高度/2
...
if __name__ == "__main__":
    ...
    print(best_fit_cylinder.vector/np.linalg.norm(best_fit_cylinder.vector))
